...
// 对比 langchain：那边消息是 HumanMessage/AIMessage 类实例，这里就是 {role, content, timestamp}
// 的普通对象 —— 这正是 pi "消息可以直接 JSON.stringify 存档再读回来"的底气所在
...
// 背景知识：模型本身不会"执行"任何工具，它只会在回复里输出"我想调用 XX 工具、参数是 YY"
// （体现在 stopReason === "toolUse"），真正执行工具、把结果喂回去，全是我们这段代码的事。
console.log("\n=== 3. 手写 agent loop（stopReason === 'toolUse'） ===");
// 工具定义 = 名字 + 描述（给模型看的，决定它会不会用/怎么用）+ 参数 schema。
// TypeBox 是用 JS 对象写 JSON Schema 的库（作用类似 Python 侧的 pydantic），
// Type.Object({...}) 生成的就是发给模型的参数格式说明，之后还能用它校验模型给的参数。
const tools: Tool[] = [
  {
    name: "get_time",
    description: "获取当前时间",
...
while (true) {
  ...
  // stopReason 是模型"为什么停下来"：
  //   "toolUse"=想用工具（循环继续）  "stop"=答完了  "length"=超长被截断  "error"/"aborted"=出错/被中断
