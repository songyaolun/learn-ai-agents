...
这是本目录的收尾篇, 把前面四个独立概念拼进同一个连贯场景, 而不是分别演示:
  - subgraph.py 的能力: "总结一篇文档"本身是个两步子流程(先写初稿、再按用户风格
    精炼), 封装成一个独立的子图 summarize_doc_subgraph, 只暴露 doc/target_style 输入。
  - send_map_reduce.py 的能力: 一次要处理几篇文档运行时才知道, 用 Send 把每篇
    文档动态派发给上面那个子图, 并行执行, 而不是写死处理 N 篇。
  - store.py 的能力: 用户"喜欢什么风格的摘要"是跨会话的长期偏好——即使这是一次
    全新的 thread_id (新会话), 只要 user_id 一样, 依然记得上次的偏好设置。
  - persistence.py / quickstart.py 的能力: 每个 thread_id (一次摘要任务) 自己的
...
时间旅行 (time_travel.py) 这里没有强行拼进来: 它是独立的调试类概念(回滚重跑),
...
def fan_out_to_docs(state: OverallState) -> list[Send]:
    ...
    原因: subgraph.py 里讲过, 子图字段名如果和父图字段名相同, 状态会直接"透传"共用
