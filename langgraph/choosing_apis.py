...
对比 langgraph/functional_api.py 与 langgraph/quickstart.py:
  - quickstart.py 是 Graph API (StateGraph) 的教学: 先定义 State schema (TypedDict),
    再 add_node / add_edge / add_conditional_edges 把控制流"画"成图。
  - functional_api.py 是 Functional API (@entrypoint / @task) 的教学: 不画图、不定义
...
# 对照 quickstart.py: 同样是 State schema + add_node + add_edge 的套路。
...
# 对照 functional_api.py: 同样用 @task 封装工作单元, @entrypoint 编排。
