...
>
> **与 `ch_17_frontend_streaming.py` 的关系**:`ch_17_frontend_streaming.py`(若同目录存在,由并行任务生成)是**数据转换代码示范**——演示如何把 `graph.stream()` 产出的 chunk 转换成前端能直接消费的结构(如 SSE 事件帧、JSON 行)。本文件则是**集成路径讲解**——从工程形态角度讲"整套系统怎么搭起来"。两者互补:一个回答"每条数据长什么样、怎么转",一个回答"整个后端↔前端怎么连"。流式部分可交叉参考本仓库的 [`ch_15_streaming.py`](./ch_15_streaming.py)(系统演示 `values`/`updates`/`messages`/`custom` 四种 `stream_mode`)。
...
一个 LangGraph 应用的核心是一个编译好的图对象(`graph = builder.compile(...)`,见本仓库 [`ch_14_combo.py`](./ch_14_combo.py))。把它接到 UI,本质是解决两件事:**(1)** 通过什么协议把图的执行(尤其是流式输出)送到浏览器;**(2)** 前端用什么形态承接与渲染。业界主要有三条路径:
...
| 可无缝接入既有系统(网关、监控、灰度) | 流式数据的"最后一公里转换"需自己写(即 `ch_17_frontend_streaming.py` 那类代码) |
| 无额外框架心智负担 | Studio 那类可视化调试要另找方案 |

### 关键机制:如何把 `graph.stream()` 接到 HTTP 流
后端拿到图后,遍历 `graph.stream(...)` 的产物,逐条 `yield` 成 SSE 帧(`data: {json}\n\n`)或 WebSocket 消息。**产出结构因 `stream_mode` 而异**(详见 [`ch_15_streaming.py`](./ch_15_streaming.py)):`messages` 模式产出 `(AIMessageChunk, metadata)` 元组适合打字机效果;`updates` 模式产出 `{节点名: 增量}` 适合展示"哪个节点动了";`custom` 模式产出节点内 `get_stream_writer()` 主动推送的对象,适合进度条等中间态。`ch_17_frontend_streaming.py` 演示的正是这一层"chunk → 前端可消费帧"的转换。
...
```json
{
  "dependencies": ["."],
  "graphs": {
    "my_agent": "./ch_14_combo.py:graph"
  },
  "env": ".env"
}
```
其中 `"my_agent": "./ch_14_combo.py:graph"` 表示把 `ch_14_combo.py` 里名为 `graph` 的对象注册为一个可调用图。
...
无论选哪种形态,前端渲染的核心都是**读懂 `graph.stream()` 各模式的产出结构**(完整机制见 [`ch_15_streaming.py`](./ch_15_streaming.py),数据转换见 `ch_17_frontend_streaming.py`)。要点如下:
...
前端交叉参考:
- 打字机气泡 → 消费 `messages` 流,注意 `messages` 模式**要求节点里真的调用了一次 chat model** 才有 token(纯 Python 函数节点不产出 token,见 [`ch_15_streaming.py`](./ch_15_streaming.py) 踩坑记录)。
- 步骤/节点面板 → 消费 `updates` 流,`key` 即节点名。
- 自定义进度 → 消费 `custom` 流;`get_stream_writer()` 的导入路径是 `langgraph.config`,只能在节点执行期间调用(见 [`ch_15_streaming.py`](./ch_15_streaming.py))。
