...
本仓库里的示例 (backend.py / sandbox_backend.py / permissions.py / summarization.py ...)
...
> 不允许 agent 自己写。可用 permissions (见本仓库 permissions.py) 声明式地拒绝对共享
...
> 【直接访问宿主机】。**不要在部署的 agent 里用它们。** —— 这正是本仓库 backend.py /
> local_shell_backend.py 明确只圈在临时目录、只做本地演示的原因; 生产要执行代码请换
> 远程沙箱 (见 sandbox_backend.py)。
...
| 裸 execute | HITL 审批 + permissions 声明式护栏 (见 permissions.py) |
