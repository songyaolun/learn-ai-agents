...
- subagents (deepagents/quickstart.py): 主 agent 把某个细分子话题的调研工作
  委派给 researcher 子 agent, 委派过程和中间细节对主 agent 的上下文不可见。
- backend=FilesystemBackend (deepagents/backend.py): agent 写的调研笔记是真实
  落在磁盘上的文件, 不是只存在于这一次进程的 state 里, 关掉进程也还在。
- interrupt_on (deepagents/hitl.py): "发布报告"这个动作定义成一个有真实副作用的
  自定义工具 publish_report, 配置成需要人工 approve/reject 才能真正执行,
  跟 hitl.py 里发邮件的例子是同一套机制。
- response_format (deepagents/structured_output.py): 拿到人工批准之后, 最终结果
  仍然要求模型调用 ResearchReport 工具收尾, 变成一个字段确定的结构化对象,
  而不是自由文本——即使流程里插了一次人工审批打断, 这个约束依然要在
  system_prompt 里显式强调 (原因见 structured_output.py 文件头的踩坑说明)。

这四块能力互不冲突: interrupt_on 依赖 checkpointer 保存"暂停时的完整状态"
(跟 hitl.py 一样), backend 只影响文件工具的存储位置, subagents 和
...
# 跟 backend.py 一样, 用系统临时目录当研究笔记的真实落地位置, 不污染仓库。
...
# 结构化最终产出的形状, 跟 structured_output.py 完全一样。
...
agent = create_deep_agent(
    ...
    # interrupt 机制依赖 checkpointer 保存暂停时的状态, 跟 hitl.py 用法一致。
