...
- env 隔离的坑: LocalShellBackend 默认 inherit_env=False, 子进程拿不到宿主机的
  环境变量。实测在某些精简环境里, 不继承 env 会让 python3 找不到 encodings 模块而
  报错; 想跑通干净的 python 计算, 需要 inherit_env=True (下面结构测试就这么做的)。
...
# - inherit_env=True: 让子进程继承宿主机环境变量, 否则 python 可能起不来 (见踩坑)。
