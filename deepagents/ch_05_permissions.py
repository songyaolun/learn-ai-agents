...
对比 deepagents/ch_04_backend.py: 那里换了 backend 让 agent 能碰真实磁盘, 但只要给了
...
- "interrupt": 走 deepagents/ch_06_hitl.py 里那套 HumanInTheLoopMiddleware, 暂停等人工
...
审批"——它复用了 deepagents/ch_06_hitl.py 里那套 HumanInTheLoopMiddleware 机制, 但完全
...
#   这个"暂停"会变成一个无法恢复的死局 (跟 ch_06_hitl.py 必须配 InMemorySaver 是同一个
