...
对比 deepagents/ch_06_hitl.py: 那里只演示了 approve (放行) 和 reject (否决) 两种最基础的
...
# 和 ch_06_hitl.py 唯一的区别: allowed_decisions 从 ["approve","reject"] 扩到四种全开。
...
agent = (
    create_deep_agent(
        ...
        # interrupt 依赖 checkpointer 保存暂停状态 (跟 ch_06_hitl.py 完全一致)。
...
def run_until_interrupt(payload, config) -> dict | None:
    ...
    遍历 snapshot.tasks 找真正带 interrupts 的那个 (不硬取下标 0), 这是 ch_10_combo.py 里
    验证过更稳妥的写法。
    """
