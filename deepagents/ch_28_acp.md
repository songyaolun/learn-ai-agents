...
| 自定义 backend (ch_04_backend.py / ch_11_sandbox_backend.py / ch_25_local_shell_backend.py) | demo agent 带文件系统 + shell, 可换成隔离 backend |
| HITL / permissions (ch_06_hitl.py / ch_05_permissions.py) | ACP 协议内置 "permission requests", 编辑器侧弹审批 |
| 会话持久 (ch_26_going_to_production.md 的 checkpointer) | ACP 用 `MemorySaver()` 等 checkpointer 支撑历史回放 |
