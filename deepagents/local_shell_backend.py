...
对比一览 (三档"能不能跑命令 / 跑在哪"):
- filesystem.py / backend.py 的 StateBackend / FilesystemBackend: 只有 ls/read/
...
- sandbox_backend.py 的 LangSmithSandbox: 同样有 execute, 但命令跑在【远程一次性
...
(见 deepagents/hitl.py、deepagents/permissions.py), 否则等于把 shell 直接交给模型。
...
def build_agent_with_execute(root_dir: str):
    ...
    生产环境务必配合 interrupt_on / permissions 做人工审批 (见 permissions.py)。
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\n[安全提醒] execute 直接在宿主机跑 shell, 无隔离; 生产给 agent 开 execute "
              "必须配 HITL 人工审批 (见 deepagents/permissions.py、hitl.py)。")
    finally:
