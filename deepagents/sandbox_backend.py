...
对比 deepagents/backend.py: 那里用的 FilesystemBackend(root_dir=...) 是"同一台
...
(deepagents/filesystem.py) 或 FilesystemBackend(deepagents/backend.py) 更省事。
...
def build_sandbox_agent(sandbox_client):
    ...
    # 都会走这个远程沙箱, 而不是宿主机磁盘 —— 这正是它区别于 backend.py 的地方。
