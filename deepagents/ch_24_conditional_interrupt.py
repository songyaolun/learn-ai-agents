...
(1) 条件式人工审批: InterruptOnConfig 的 when 谓词。
    对比 deepagents/ch_06_hitl.py: 那里的 interrupt 是"无条件"的——只要模型调用了
...
(2) 实验性的 v3 事件流 (astream_events version="v3") 骨架。
    对比 deepagents/ch_07_stream.py: 那里用的是稳定的 stream(stream_mode=[...], version="v2"),
    结论是 v2 的 updates 流里看不到 subagent 内部逐步执行 (子 agent 整轮被封装成一次
    task 工具结果)。ch_07_stream.py 文件头提到有个实验性的 stream_events(v3) + interleave API
...
# 对比 ch_06_hitl.py 的 {"send_email": {"allowed_decisions": [...]}} (无条件、每次都拦),
# 这里是"金额 > 阈值才拦, 否则自动放行"。
agent = (
    create_deep_agent(
        model=model,
        ...
        # 只要可能触发 interrupt, 就必须配 checkpointer 保存暂停状态 (同 ch_06_hitl.py)。
...
#
# 这是 ch_07_stream.py 文件头提到、但没演示的实验性 API。astream_events(version="v3") 方法
