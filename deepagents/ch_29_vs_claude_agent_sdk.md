...
> 这正好对应本仓库的 backend 谱系: ch_03_filesystem.py (虚拟 state) → ch_04_backend.py
> (FilesystemBackend 真实磁盘) → ch_25_local_shell_backend.py (LocalShellBackend 本地 shell)
> → ch_11_sandbox_backend.py (远程隔离沙箱)。"可插拔 backend"就是 Deep Agents 相对 Claude
> Agent SDK 最直观的灵活性体现。
