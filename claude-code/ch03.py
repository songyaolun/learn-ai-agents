...
PLAN_REMINDER_INTERVAL = 3  # 每隔多少轮没有更新计划就提醒一次
...
class PlanItem:
    ...
    active_form: str = ""  # active form


@dataclass
class PlanningState:
    items: list[PlanItem] = field(default_factory=list)
    rounds_since_last_update: int = 0
...
class TodoManager:
    ...
    def update(self, items: list) -> str:
        ...
        for index, raw_item in enumerate(items):
            ...
            if not content:
                raise ValueError(f"Plan item {index + 1} is missing content")
        ...
        self.state.rounds_since_last_update = 0
        return self.render()

    def note_round_without_update(self) -> None:
        self.state.rounds_since_last_update += 1

    def reminder(self) -> str | None:
        if not self.state.items:
            return None
        if self.state.rounds_since_last_update < PLAN_REMINDER_INTERVAL:
            return None
    ...
    def render(self) -> str:
        ...
        for item in self.state.items:
            ...
            if item.status == "in_progress" and item.active_form:
                line += f" (active form: {item.active_form})"
            lines.append(line)

        completed = sum(1 for item in self.state.items if item.status == "completed")
        lines.append(f"\nProgress: {completed}/{len(self.state.items)} completed")
        return "\n".join(lines)
...
def run_one_turn(state: LoopState) -> bool:
    ...
    for block in response.content:
        ...
        if getattr(block, "name", None) == "todo":
            used_todo = True

    if used_todo:
        TODO.state.rounds_since_last_update = 0
    else:
