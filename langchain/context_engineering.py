...
对比 langchain/quickstart.py: 那里用 create_agent 封装了上下文管理；这里显式展示上下文的组装与传递机制，核心区别是抽象层级（黑盒封装 vs 显式控制）和可扩展性（固定流程 vs 自定义注入）。
...
2. 三种数据来源：运行时上下文（runtime context，呼应 runtime.py）、状态（state）、存储（store，呼应 long_term_memory.py）
3. 上下文组装与传递：从三种来源取数据并注入到模型/工具中
4. 踩坑记录：参数存在但模型不一定按预期使用

本文件串联了 models.py（模型初始化）、messages.py（消息构造）、long_term_memory.py（存储）、runtime.py（运行时）四个已有示例，展示了如何从这四个模块中获取数据并构建上下文。
"""
...
# 呼应 runtime.py 的运行时管理和 long_term_memory.py 的存储机制
def assemble_context(user_id: str, session_id: str) -> AgentContext:
    """从三种数据来源组装完整的上下文"""
    # 运行时上下文（runtime context）：来自 runtime.py 的运行时配置
    runtime_config = {
        "max_steps": 5,
        "timeout": 30,
    ...
    # 存储（store）：来自 long_term_memory.py 的长期记忆
    ...
    # 模型上下文：来自 models.py 的模型配置和 system prompt
    model_context = {
...
def get_user_preference(context: AgentContext, preference_key: str) -> str:
    ...
    # 读取长期记忆（呼应 long_term_memory.py 的存储操作）
